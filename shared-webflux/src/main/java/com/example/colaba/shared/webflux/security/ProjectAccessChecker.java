...
public class ProjectAccessChecker {
    ...
    public Mono<ProjectRole> getUserProjectRoleMono(Long projectId, Long userId) {
        return client.getUserProjectRole(projectId, userId)
                .mapNotNull(roleName -> roleName == null ? null : ProjectRole.valueOf(roleName));
    }
        ...
        return isAtLeastEditorMono(projectId, userId)
                .flatMap(isEditor -> {
                    ...
                    return getUserProjectRoleMono(projectId, userId)
                            .map(role -> role == null ? "none" : role.name())
                            .defaultIfEmpty("none")
                            .flatMap(roleStr -> Mono.error(new AccessDeniedException(
                                    "Required project role: at least EDITOR. Current role: " + roleStr)));
                });
    }
